...
// Tu código acá:
let test = document.querySelector('#createdBy')
test.innerHTML += 'El leon';
...
function displayToDos() {
  ...
  let cinco = buildToDos(toDoItems)
  for (let i = 0; i < cinco.length; i++) {
    toDoContainer.appendChild(cinco[i])
  }
}
...
function addToDo() {
  // Tu código acá:
  let random = document.getElementById('toDoInput')
  let gonza = new ToDo(random.value)
  toDoItems.push(gonza)
  random.value = ''
  displayToDos()
}
...
// Tu código acá:
let bartolomiau = document.getElementById('addButton');
bartolomiau.addEventListener('click', addToDo);
